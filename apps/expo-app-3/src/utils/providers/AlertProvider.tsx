...
const AlertProvider = ({ children }: { children: ReactNode }) => {
  ...
  alertFnRef.current = (title: string, message?: string, options?: MyAlertButton[], cancelTitle?: string) =>
    setState({
      visible: true,
      title,
      message,
  ...
  const buttons = useMemo(() => {
    ...
    if (!state.options?.some((o) => o.style === 'cancel')) {
      const calculatedCancelText = state.cancelTitle || (state.options ? 'Cancel' : 'Ok');
  ...
  return (
    <AlertContext.Provider value={{ state, setState }}>
      <CenteredModal title={state.title} visible={state.visible} onRequestClose={() => handleSelection()}>
            ...
            {buttons?.map(({ text, style, icon }, i) => {
              return (
                <Button
                  ...
                  style={styles.button}
                  accessoryLeft={icon ? (props) => <Icon name={icon} {...props} /> : undefined}
                />
              );
            })}
          </View>

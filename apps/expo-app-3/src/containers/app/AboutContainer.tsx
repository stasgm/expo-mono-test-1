...
const elements = [
  h4('Privacy Policy'),
  p('You can find the full privacy policy for this app ', a(PRIVACY_URL, 'here'), '.'),
  h4('Acknowledgements'),
  p('Thanks to the following open source software and free services for making this project possible.'),
  [
  ...
  ].map((link) => <Fragment key={link.name}>{p(a(link.url, link.name))}</Fragment>),
  p(a(`${MyConstants.githubUrl}/blob/master/package.json`, 'And many more... ', { showIcon: true })),
  h4("Who's building this?"),
  p('You can find out more about the developer ', a('mrarich.com/about', 'here'), '.'),
  p(),
  p(
  ...
  p(),
  p(`Version ${MyConstants.version} © ${new Date(Date.now()).getFullYear()} Alex Rich`),
];

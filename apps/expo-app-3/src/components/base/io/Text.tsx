...
const Text = ({
  ...
  if (right) {
    style = StyleSheet.flatten([style, { textAlign: 'right' }]);
  }
  if (italic) {
  ...
  if (mono) {
    style = StyleSheet.flatten([{ fontFamily: 'RobotoMono_400Regular' }, style]);
  }

  return (
    <UIKText selectable {...props} category={category} appearance={appearance} style={style}>
      {children}
    </UIKText>
  );
};
...
const p = (...args: (ReactText | TextElement)[]) => (
  <Text style={styles.p} selectable>
    {args.every((n) => typeof n === 'string') ? Array.prototype.join.call(args, '\n\n') : keyed(args)}
  </Text>
);

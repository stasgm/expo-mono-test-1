...
import { Divider, TopNavigation as UIKTopNavigation, TopNavigationAction } from '@ui-kitten/components';
...
export default <T extends NativeStackHeaderProps | DrawerHeaderProps>(topInsets: number) =>
  ({ options, navigation, route, ...others }: T) => {
    ...
    if ('pop' in navigation && 'back' in others) {
      ...
      BackAction = () => (
        <TopNavigationAction icon={(props) => <Icon {...props} name={Icons.arrowBack} />} onPress={goBack} />
      );
    }
    ...
    const renderRightActions = () => {
      ...
      if (headerRight) {
        headerActions.push(<Fragment key="action">{headerRight({ canGoBack })}</Fragment>);
      }

...
const DropdownPicker = forwardRef(
  <T extends string | number>(
    { onValueChange, selectedValue, options, label, description, icon, style }: Props<T>,
    ref?: Ref<Select | null>,
  ) => {
    ...
    return (
      <View style={style}>
        {label ? <Label label={label} icon={icon} tooltip={description} style={styles.label} /> : null}
        ...
        >
          {options.map(({ value, label: optionLabel, icon: optionIcon }) => (
            <SelectItem
              key={value}
              title={optionLabel}
              accessoryRight={optionIcon ? (props) => <Icon name={optionIcon} {...props} /> : undefined}
            />
          ))}
        </Select>
      </View>

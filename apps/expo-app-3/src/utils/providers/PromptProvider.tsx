...
import { Button, CenteredModal, DropdownPicker, PickerOption, Text, View } from '../../components/base';
...
const PromptProvider = ({ children }: { children: ReactNode }) => {
  ...
  const onClosePrompt = useCallback(
    (cancelled: boolean) => state.callback && state.callback(textValue, cancelled),
    [state, textValue],
  );
      ...
      new Promise((resolve) => {
        const cb = (value: string, cancelled: boolean) => {
          if (settings?.maxLength && settings.maxLength < value.length) {
            alert('Too long!', `Please remove at least ${value.length - settings.maxLength} characters`);
            return;
          }
  ...
  return (
    <PromptContext.Provider value={{ state, setState }}>
      ...
      >
        ...
        {renderInput()}
        <View row spread>
          <Button outline status="basic" label="Cancel" onPress={() => setState({ visible: false })} />
          <Button label={state.okButton || 'Ok'} onPress={() => onClosePrompt(false)} />
        </View>
      </CenteredModal>
      {children}

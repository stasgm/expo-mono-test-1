...
export default ({ data, hideEmptyValues = true, style }: Props) => {
  const filteredData = hideEmptyValues ? data.filter(({ value }) => typeof value !== 'undefined') : data;
  return (
    <View style={style}>
      {filteredData.map(({ value = '', link, ...labelProps }, i) => (
        <Fragment key={labelProps.label}>
          <View style={styles.item}>
            <Label {...labelProps} />
            <Text>
              {link && typeof value === 'string' ? a(value, getUrlDisplay(value), { showIcon: true }) : value}
            </Text>
          </View>

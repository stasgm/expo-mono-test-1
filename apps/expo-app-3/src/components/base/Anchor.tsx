...
const Anchor = ({ url, text, showIcon, onPress: onPressProp, status = 'primary', ...textProps }: Props) => {
    ...
    <Text status={status} onPress={onPress} {...textProps}>
      ...
      <>
        {showIcon ? (
          <>
            {'    '}
            <Icon name={Icons.externalLink} width={iconSize} height={iconSize} fill={color} />
          </>
        ) : undefined}
      </>
    </Text>
...
export const a = (url: string, text: string, options: Omit<Props, 'url' | 'text'> = {}) => {

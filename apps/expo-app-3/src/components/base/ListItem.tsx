...
const ListItem = ({
  ...
  const accessoryLeft: Props['accessoryLeft'] = icon ? (props) => <Icon name={icon} {...props} /> : undefined;
  const accessoryRight: Props['accessoryRight'] = (props) => (
    <View style={styles.row}>
      {badge ? <Badge {...badge} /> : undefined}
      {rightIcon ? typeof rightIcon === 'string' ? <Icon {...props} name={rightIcon} /> : rightIcon : undefined}
      {typeof chevronOverride === 'string' || typeof chevronOverride === 'undefined' ? (
        <Icon {...props} name={chevronOverride || Icons.chevronRight} />
      ) : undefined}
    </View>
  );

  const titleComp = typeof title === 'string' ? <Text numberOfLines={titleNumberOfLines}>{title}</Text> : title;

...
type AlertFn = (title: string, message?: string, options?: AlertButton[], cancelTitle?: string) => void;
 ...
 */
export const prompt: PromptFn = (title, description, settings) => promptFnRef.current(title, description, settings);
 ...
 */
export const toast: ToastFn = (message, status, timeout = 3000) => toastFnRef.current(message, status, timeout);

const hasMessage = (error: any): error is { message: string } => error && 'message' in error;

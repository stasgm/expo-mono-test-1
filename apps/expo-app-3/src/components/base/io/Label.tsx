import { PopoverPlacements, Text, TextProps, Tooltip } from '@ui-kitten/components';
...
const Label = ({ label, icon, tooltip, ...textProps }: Props) => {
  ...
  return (
    <Tooltip
      anchor={() => (
        <Pressable style={styles.row} onPress={tooltip ? () => setTooltipVisible(true) : undefined}>
          ...
          {computedIcon ? (
            <>
              <Text> </Text>
              <Icon name={computedIcon} width={iconSize} height={iconSize} fill={iconFill} />
            </>
          ) : undefined}
        </Pressable>
      )}
